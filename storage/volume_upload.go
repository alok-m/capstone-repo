...
func parseMultipart(r *http.Request, sizeLimit int64, pu *ParsedUpload) (e error) {
	...
	if fe != nil {
		glog.V(11).Infoln("MultipartReader [ERROR]", fe)
		e = fe
		return
	}
	...
	if fe != nil {
		glog.V(11).Infoln("Reading Multi part [ERROR]", fe)
		e = fe
		return
	}
	...
	if e != nil {
		glog.V(11).Infoln("Reading Content [ERROR]", e)
		return
	}
	...
	for pu.FileName == "" {
		...
		if fName != "" {
			...
			if fe2 != nil {
				glog.V(11).Infoln("Reading Content [ERROR]", fe2)
				e = fe2
				return
			}
